...
import { webAlertsAPI, webFilteredAlertsAPI } from "../../api/alertsAPI";
import { alertOrganiser, attackCalculator, ruleCalculator, severityCalculator } from "../../utils/webAnalysisUtils";
...
const WebAnalysis = () => {
  ...
  const { data:filteredAlerts , isSuccess : filterDataFetched , isPending : filteredAlertsPending , isError : isFiltererdError , error : filteringError } = useQuery({
    queryKey: ["webFilteredAlerts",filterData],
    queryFn: ()=>webFilteredAlertsAPI(filterData)  ,
    retry : false ,
  ...
  useEffect(() => {
    document.title = "Web Analysis Result";
  }, []);
  ...
  const rulesData = {
    labels: ["WEB-001", "WEB-002", "WEB-003", "WEB-004"],
    datasets : [{
      data: [ruleValues["WEB-001"], ruleValues["WEB-002"], ruleValues["WEB-003"], ruleValues["WEB-004"]],
      backgroundColor: ["#5a098d","#f8026d","#069df5", "#0d6805"],
      borderRadius: 4,
      barPercentage: 0.7 ,
  ...
  const attackData = {
    labels: [
      ...
      "SENSITIVE FILE ACCESS ATTACK", 
      "SQL INJECTION ATTACK" ,
      "WEB SHELL ACCESS ATTACK" ,
    ],
    datasets : [{
      data: [
        attackValues["Directory Enumeration"],
        attackValues["Sensitive File Access"],
        attackValues["SQL Injection Attempt"],
        attackValues["Web Shell Access Detection"],
      ],
      backgroundColor: ["#5a098d","#f8026d","#069df5", "#0d6805"],
      borderRadius: 2,
      barPercentage: 0.4 ,
              ...
              <section>
                ...
                <div>
                  ...
                  <select name="attack" onChange={handleChange} defaultValue={filterData.attack} style={{ width : "400px" }}>
                    ...
                    {["Directory Enumeration","Sensitive File Access" ,"SQL Injection Attempt" ,"Web Shell Access Detection"].map((attack)=>{
                      ...
                      {alerts.map((alert)=>{
                        return (
                          <tr key={alert._id}>
                            ...
                            <td><Eye size={15} className="lui-eye" onClick={()=>navigate(`/web/analysis/${alert.path}/${alert._id}`)}/></td>
                          </tr>
                        )
                      })}
                    </tbody>
